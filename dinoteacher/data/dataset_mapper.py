...
def album_extra_augmentation(image: np.array, target_list: list[str],  p: float = 0.5):
    aug = A.Compose(
                [A.OneOf(
                    [A.HistogramMatching(target_list, p=1.0),
                     A.FDA(target_list, p=1.0),
                     A.PixelDistributionAdaptation(target_list, p=1.0)])], p=p)
...
class DatasetMapperTwoCropSeparateKeepTf(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True, keep_tf_data=False):
        ...
        self.album_aug = cfg.SEMISUPNET.ALBUM_AUG
        
        self.album_aug_p = cfg.SEMISUPNET.ALBUM_AUG_P

        
        target_list_dirs = []
        target_list_dirs.extend(cfg.SEMISUPNET.ALBUM_TARGET_LIST)
        self.target_list = []
        for target_list_dir in target_list_dirs:
            for target_image in os.listdir(target_list_dir):
                self.target_list.append(os.path.join(target_list_dir, target_image))
    ...
    def __call__(self, dataset_dict):
        ...
        if(self.album_aug == True):
            
            
            image_weak_aug = album_extra_augmentation(image=image_weak_aug, target_list=self.target_list, p=self.album_aug_p)
        ...
        if self.img_format == "BGR":
            image_weak_rgb = image_weak_aug[:, :, ::-1].copy()  
        else:
            image_weak_rgb = image_weak_aug
    
        image_pil = Image.fromarray(image_weak_rgb.astype("uint8"), "RGB")
        img_strong = self.strong_augmentation(image_pil)
        image_strong_aug = np.array(img_strong)
        
        if self.img_format == "BGR":
            image_strong_aug = image_strong_aug[:, :, ::-1].copy()
...
class DatasetMapperTwoCropSeparateKeepTf_lt(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True, keep_tf_data=False):
        ...
        if(cfg.SEMISUPNET.FEATURE_STRIDE == 32):
            self.LCM = 224
        elif(cfg.SEMISUPNET.FEATURE_STRIDE == 16):
            self.LCM = 112
        else:
        ...
        if(self.album_aug == True):
            ...
            self.target_dir = cfg.SEMISUPNET.ALBUM_TARGET_DIR
            self.target_list = [os.path.join(self.target_dir, target_image) for target_image in os.listdir(self.target_dir)]        
            
            self.domain_aug = A.Compose(
                        [A.FDA(self.target_list, p=1.0, beta_limit=(0.05, 0.2))],
                        p=self.album_aug_p)
    ...
    def __call__(self, dataset_dict):
        ...
        if self.album_aug:
            image_weak_aug = self.domain_aug(image=image_weak_aug)["image"]
        ...
        LCM = self.LCM
        h, w = image_weak_aug.shape[:2]
        pad_h = (LCM - h % LCM) % LCM
        pad_w = (LCM - w % LCM) % LCM
        if pad_h > 0 or pad_w > 0:
            
            image_weak_aug = np.pad(
                image_weak_aug,
                ((0, pad_h), (0, pad_w), (0, 0)),  
                mode='constant',
                constant_values=0.0
