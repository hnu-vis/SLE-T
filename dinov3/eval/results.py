...
def save(
    ...
    assert file_format is not None
    if file_format == FileFormat.CSV:
        results.to_csv(f, index=False, header=True, sep=",", na_rep="")  
    elif file_format == FileFormat.JSON_LINES:
        
        s = results.to_json(orient="records", lines=True, indent=None)  
        if isinstance(f, (str, PathLike)):
            context = open(f, "w")  
        else:
            context = nullcontext(enter_result=f)  
        with context as f:
            for line in s.splitlines():
                line = json.dumps(json.loads(line), separators=(",", ":"))
                f.write(line + "\n")
    elif file_format == FileFormat.EXCEL:
        results.to_excel(f, header=True, index=False, na_rep="")  
    elif file_format == FileFormat.YAML:
        with open(f, "w") as fp:  
