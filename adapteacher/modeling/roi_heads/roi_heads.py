...
from adapteacher.modeling.roi_heads.fast_rcnn import FastRCNNFocaltLossOutputLayers, FastRCNNOutputLayers_smooth_label
...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeadsPseudoLab_smooth_label(StandardROIHeads):
    @classmethod
    def _init_box_head(cls, cfg, input_shape):
        ...
        if cfg.MODEL.ROI_HEADS.LOSS == "CrossEntropy_LS":
            box_predictor = FastRCNNOutputLayers_smooth_label(cfg, box_head.output_shape)
        elif cfg.MODEL.ROI_HEADS.LOSS == "FocalLoss":
            box_predictor = FastRCNNFocaltLossOutputLayers(cfg, box_head.output_shape)
        else:
            raise ValueError("Unknown ROI head loss.")
